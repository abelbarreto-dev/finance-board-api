    ...
    async updateOperaBank(operaBankId: number, operaBankDTO: OperaBankDTO): Promise<OperaBank> {
        try {
            ...
            return await this.prisma.$transaction(async () => {
                ...
                const operaBank = await this.prisma.operaBank.findFirst({
                    where: {id: operaBankId}
                }).then(operaBank => {
                    if (operaBank) return operaBank as unknown as OperaBank;
                    throw new DatabaseException("opera bank not found", 404);
                });

                operaBank.description = operaBankDTO.description;
                operaBank.typeTransfer = operaBankDTO.typeTransfer;
                operaBank.state = operaBankDTO.state;
                operaBank.bankOperation = operaBankDTO.bankOperation;
                operaBank.senderName = operaBankDTO.senderName;
                operaBank.receiverName = operaBankDTO.receiverName;
                operaBank.balanceValue = operaBankDTO.balanceValue;

                bank.balanceValue = await this.bankBalanceOperation(
                    bank.balanceValue,
                    operaBank.balanceValue,
                    operaBank.bankOperation
                );
                ...
                return await this.prisma.operaBank.update({
                    data: operaBank,
                    where: {id: operaBankId}
                }) as unknown as OperaBank;
            });
        }
        catch (error: unknown) {
            console.error(error);

            throw new DatabaseException("error to update opera bank at database", 500);
        }
        finally {
            this.prisma.$disconnect();
        }
