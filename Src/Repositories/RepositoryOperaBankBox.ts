...
export class RepositoryOperaBankBox {
    ...
    async updateOperaBankBox(operaBankBoxId: number, operaBankBoxDTO: OperaBankBoxDTO): Promise<OperaBankBox> {
        try {
            this.prisma.$connect();

            return await this.prisma.$transaction(async () => {
                const bankBox = await this.prisma.bankBox.findFirst({
                    where: {id: operaBankBoxDTO.bankBoxId}
                }).then(bankBox => {
                    if (bankBox) return bankBox as unknown as BankBox;
                    throw new DatabaseException("bank box not found", 404);
                });

                const operaBankBox = await this.prisma.operaBankBox.findFirst({
                    where: {id: operaBankBoxId}
                }).then(operaBankBox => {
                    if (operaBankBox) return operaBankBox as unknown as OperaBankBox;
                    throw new DatabaseException("opera bank box not found", 404);
                });

                operaBankBox.bankBoxId = operaBankBoxDTO.bankBoxId;
                operaBankBox.typeTransfer = operaBankBoxDTO.typeTransfer;
                operaBankBox.balanceValue = operaBankBoxDTO.balanceValue;
                ...
                return await this.prisma.operaBankBox.update({
                    data: operaBankBox,
                    where: {id: operaBankBoxId}
                }) as unknown as OperaBankBox;
            });
        }
        catch (error: unknown) {
            console.error(error);

            throw new DatabaseException("error to update opera bank box at database", 500);
        }
        finally {
            this.prisma.$disconnect();
        }
